        ...
        <div class="box">
            @isset($portfolio->about->logo)
                <div class="avatar rounded-circle overflow-hidden" style="width:68px; height:68px;">
                    <img src="{{ Storage::url($portfolio->about->logo) }}" alt="avatar" class="w-100 h-100 object-fit-cover">
                </div>
            @endisset
            ...
            <ul class="nav-menu style-3 ">
                ...
                @foreach ($portfolio->sectionOrders->sortBy('position') as $sectionOrder)
                    @switch($sectionOrder->section_id)
                        ...
                        @case('skills')
                            <li class="text-menu text_white">
                                <a href="#skills" class="nav_link toggle splitting link link-no-action text-button font-3 fw-6">
