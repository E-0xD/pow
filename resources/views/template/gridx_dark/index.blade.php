...
<html lang="en">
    ...
    <head>
        ...
        <link rel="stylesheet" href="{{ asset('template_assets/gridx/css/style.css') }}">
    </head>
    ...
    <body>
        ...
        <main class="main-homepage">
            ...
            <!-- Header -->
            @include('template.gridx_dark.header')

            @include('template.gridx_dark.user_bar')
            ...
            @foreach ($otherSections as $sectionOrder)
                @switch($sectionOrder->section_id)
                    @case('about')
                        @include('template.gridx_dark.about')
                    @break

                    @case('skills')
                        @include('template.gridx_dark.skills')
                    @break

                    @case('projects')
                        @include('template.gridx_dark.projects')
                    @break
                @endswitch
                                ...
                                <div class="row mt-24">
                                    @foreach ($experienceEducation as $expEdu)
                                        @switch($expEdu->section_id)
                                            @case('experience')
                                                @include('template.gridx_dark.experience')
                                            @break

                                            @case('education')
                                                @include('template.gridx_dark.education')
                                            @break
                                        @endswitch
                                    @endforeach
                                </div>
            ...
            @if ($portfolio->accept_messages)
                @include('template.gridx_dark.message')
            @endif
