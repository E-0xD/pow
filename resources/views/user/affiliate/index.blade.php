    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
            <div class="flex flex-col gap-1">
                <h1 class="text-3xl font-bold tracking-tight text-text-light dark:text-text-dark">Affiliate Dashboard
                </h1>
                <p class="text-base font-normal leading-normal text-text-secondary-light dark:text-text-secondary-dark">
                    Monitor your performance and earnings.</p>
            </div>
            <button
                class="flex items-center gap-2 min-w-[84px] cursor-pointer justify-center overflow-hidden rounded-lg h-10 px-4 bg-card-light dark:bg-card-dark border border-border-light dark:border-border-dark text-text-light dark:text-text-dark text-sm font-medium leading-normal">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
            <div
                ...
                <h2 class="text-[#141118] dark:text-white text-[22px] font-bold leading-tight tracking-[-0.015em] mb-4">
                    Your Referral Link</h2>
                <div class="flex flex-col sm:flex-row items-stretch gap-4">
                    <div class="flex w-full flex-1 items-stretch rounded-xl">
                        <input
                            class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-[#141118] dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary/50 border border-[#e0dbe6] dark:border-gray-700 bg-background-light dark:bg-black/20 h-14 placeholder:text-[#756189] p-[15px] text-base font-normal leading-normal"
                            readonly="" value="{{ url('/?data=' . ($affiliate->uid ?? '')) }}" />
                    </div>
                    <button
                        class="flex min-w-[140px] cursor-pointer items-center justify-center overflow-hidden rounded-xl h-14 px-4 bg-primary text-white text-sm font-bold leading-normal tracking-[0.015em] hover:bg-primary/90 transition-colors">
            ...
            <div
                ...
                <div>
                    <h2 class="text-[#141118] dark:text-white text-[22px] font-bold leading-tight tracking-[-0.015em] mb-2">Payouts</h2>
                    <p class="text-[#756189] dark:text-gray-400 text-sm mb-4">Payouts are processed automatically every {{ config('affiliate.payout_interval_days', 30) }} days.</p>
                </div>
                ...
                <form action="{{ route('user.affiliate.store') }}" method="post">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <button type="submit" class="mt-2 w-full rounded-xl h-10 bg-primary text-white">Save Payout Details</button>
                    </div>
                        ...
                        <tbody>
                            @foreach($transactions as $t)
                                <tr class="border-b border-border-light dark:border-border-dark">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="px-2 py-1 text-xs font-medium rounded-full @if($t->status === 'successful') bg-green-100 text-green-800 @elseif($t->status === 'pending') bg-yellow-100 text-yellow-800 @else bg-gray-100 text-gray-800 @endif">{{ ucfirst($t->status) }}</span>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
