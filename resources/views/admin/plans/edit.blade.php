...
<div class="mx-auto max-w-4xl px-4 py-8">
    <h1 class="text-text-light dark:text-text-dark text-3xl font-black tracking-tighter mb-6">Edit Plan</h1>
    ...
    <div class="bg-card-light dark:bg-card-dark shadow-md rounded-lg p-6 border border-border-light dark:border-border-dark">
        <form method="POST" action="{{ route('admin.plan.update', $plan) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $plan->name) }}" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary" required>
            ...
            <div class="mb-4">
                ...
                <select name="tier_id" id="tier_id" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary" required>
                    ...
                    @foreach($tiers as $tier)
                        <option value="{{ $tier->id }}" {{ old('tier_id', $plan->tier_id) == $tier->id ? 'selected' : '' }}>
                            {{ $tier->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary">{{ old('description', $plan->description) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <input type="number" name="price" id="price" value="{{ old('price', $plan->price) }}" step="0.01" min="0" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary">
            ...
            <div class="mb-4">
                ...
                <select name="billing_cycle" id="billing_cycle" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary" required>
                    <option value="monthly" {{ old('billing_cycle', $plan->billing_cycle->value) == 'monthly' ? 'selected' : '' }}>Monthly</option>
                    <option value="yearly" {{ old('billing_cycle', $plan->billing_cycle->value) == 'yearly' ? 'selected' : '' }}>Yearly</option>
                </select>
                @error('billing_cycle') <p class="text-background-danger text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label for="duration" class="block text-sm font-medium text-text-light dark:text-text-dark">Interval (Days)</label>
                <input type="number" name="duration" id="duration" value="{{ old('duration', $plan->duration) }}" min="1" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="paystack_plan_code" id="paystack_plan_code" value="{{ old('paystack_plan_code', $plan->paystack_plan_code) }}" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary">
                @error('paystack_plan_code') <p class="text-background-danger text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="mb-4">
                <label class="inline-flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $plan->is_active) ? 'checked' : '' }} class="rounded border-border-light dark:border-border-dark text-primary shadow-sm focus:ring-primary">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="bg-primary hover:opacity-90 text-text-dark font-bold py-2 px-4 rounded">Update Plan</button>
            </div>
        </form>
    </div>
</div>
