...
<div class="mx-auto max-w-4xl px-4 py-8">
    <h1 class="text-text-light dark:text-text-dark text-3xl font-black tracking-tighter mb-6">Edit Tier</h1>

    <div class="bg-card-light dark:bg-card-dark shadow-md rounded-lg p-6 border border-border-light dark:border-border-dark">
        <form method="POST" action="{{ route('admin.tier.update', $tier) }}">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-text-light dark:text-text-dark">Tier Name</label>
                <input type="text" name="name" id="name" value="{{ old('name', $tier->name) }}" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary" required>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $tier->slug) }}" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary" required>
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="3" class="mt-1 block w-full border border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark text-text-light dark:text-text-dark rounded-md shadow-sm focus:ring-primary focus:border-primary">{{ old('description', $tier->description) }}</textarea>
            ...
            <div class="mb-6">
                ...
                <div class="space-y-3 bg-background-light dark:bg-background-dark p-4 rounded-md border border-border-light dark:border-border-dark">
                    @forelse ($features as $feature)
                        @php
                            $tierFeature = $tier->features()->where('feature_id', $feature->id)->first();
                            $isEnabled = $tierFeature ? true : false;
                            $value = $tierFeature ? $tierFeature->pivot->value : '';
                        @endphp
                        <div class="flex items-start gap-3">
                            <input type="checkbox" 
                                ...
                                value="1" 
                                {{ $isEnabled ? 'checked' : '' }}
                                class="mt-1 rounded border-border-light dark:border-border-dark text-primary shadow-sm focus:ring-primary">
                            <div class="flex-1">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="bg-primary hover:opacity-90 text-text-dark font-bold py-2 px-4 rounded">Update Tier</button>
            </div>
        </form>
    </div>
</div>
